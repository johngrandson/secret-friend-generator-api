"""ApprovePlanUseCase — approve a pending plan version."""
...
class ApprovePlanUseCase:
    ...
    async def execute(self, request: ApprovePlanRequest) -> ApprovePlanResponse:
        async with self._uow:
            plan = await self._uow.plans.find_by_id(request.plan_id)
            if plan is None:
                return ApprovePlanResponse(None, False, "Plan not found.")
            try:
                plan.approve(by=request.approved_by)
            except ValueError as exc:
                return ApprovePlanResponse(None, False, str(exc))
        ...
        return ApprovePlanResponse(PlanDTO.from_entity(updated), True)
