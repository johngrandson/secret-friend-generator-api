...
class ServerSession(
    ...
    def check_client_capability(
        ...
        if capability.sampling is not None:
            if client_caps.sampling is None:
                return False
            if (
                capability.sampling.context is not None
                and client_caps.sampling.context is None
            ):
                return False
            if (
                capability.sampling.tools is not None
                and client_caps.sampling.tools is None
            ):
                return False
    ...
    def _build_elicit_form_request(
        ...
        request_id = (
            f"task-{related_task_id}-{id(params)}"
            if related_task_id
