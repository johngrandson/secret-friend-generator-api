...
def get_client_metadata_scopes(
    ...
    if www_authenticate_scope is not None:
        ...
        and protected_resource_metadata.scopes_supported is not None
    ):
        # Priority 2: PRM scopes_supported
        return " ".join(protected_resource_metadata.scopes_supported)
    elif (
        authorization_server_metadata is not None
...
def build_oauth_authorization_server_metadata_discovery_urls(
...
async def handle_auth_metadata_response(
    response: Response,
...
async def handle_registration_response(
    response: Response,
) -> OAuthClientInformationFull:
