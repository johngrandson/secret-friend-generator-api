...
class ClientSession(
    ...
    @overload
    @deprecated(
        "Use list_resource_templates(params=PaginatedRequestParams(...)) instead"
    )
    ...
    async def list_resource_templates(
        ...
        *,
        ...
        return await self.send_request(
            types.ClientRequest(
                types.ListResourceTemplatesRequest(params=request_params)
            ),
            types.ListResourceTemplatesResult,
        )
    ...
    async def send_roots_list_changed(self) -> None:  # pragma: no cover
        ...
        await self.send_notification(
            types.ClientNotification(types.RootsListChangedNotification())
        )
        ...
        match responder.request.root:
            case types.CreateMessageRequest(params=params):
                with responder:
                    ...
                    if params.task is not None:
                        response = await self._task_handlers.augmented_sampling(
                            ctx, params, params.task
                        )
                    else:
            ...
            case types.ElicitRequest(params=params):
                with responder:
                    # Check if this is a task-augmented request
                    if params.task is not None:
                        response = await self._task_handlers.augmented_elicitation(
                            ctx, params, params.task
                        )
                    else:
