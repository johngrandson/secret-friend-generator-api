...
class OAuthClientProvider(httpx.Auth):
    ...
    async def _validate_resource_match(self, prm: ProtectedResourceMetadata) -> None:
        ...
        if not check_resource_allowed(
            requested_resource=default_resource, configured_resource=prm_resource
        ):
    ...
    async def _handle_protected_resource_response(
        ...
        if response.status_code == 200:
            try:
                ...
                logger.warning(
                    f"Invalid protected resource metadata at {response.request.url}"
                )
                return False
        elif response.status_code == 404:  # pragma: no cover
            # Not found - try next URL in fallback chain
            logger.debug(
                f"Protected resource metadata not found at {response.request.url}, trying next URL"
            )
            return False
        else:
    ...
    async def _refresh_token(self) -> httpx.Request:
        ...
        if self.context.oauth_metadata and self.context.oauth_metadata.token_endpoint:
            token_url = str(
                self.context.oauth_metadata.token_endpoint
            )  # pragma: no cover
        else:
            auth_base_url = self.context.get_authorization_base_url(
                self.context.server_url
    ...
    def _add_auth_header(self, request: httpx.Request) -> None:
        ...
        if (
            self.context.current_tokens and self.context.current_tokens.access_token
        ):  # pragma: no branch
    ...
    async def async_auth_flow(
        ...
        async with self.context.lock:
            ...
            if not self.context.is_token_valid() and self.context.can_refresh_token():
                ...
                if not await self._handle_refresh_response(
                    refresh_response
                ):  # pragma: no cover
