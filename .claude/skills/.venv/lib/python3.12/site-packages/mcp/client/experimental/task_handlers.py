...
class ExperimentalTaskHandlers:
    ...
    get_task_result: GetTaskResultHandlerFnT = field(
        default=default_get_task_result_handler
    )
    ...
    augmented_sampling: TaskAugmentedSamplingFnT = field(
        default=default_task_augmented_sampling
    )
    augmented_elicitation: TaskAugmentedElicitationFnT = field(
    ...
    def build_capability(self) -> types.ClientTasksCapability | None:
        ...
        has_elicitation = (
            self.augmented_elicitation is not default_task_augmented_elicitation
        )
        ...
        if has_sampling or has_elicitation:
            requests_capability = types.ClientTasksRequestsCapability(
                sampling=types.TasksSamplingCapability(
                    createMessage=types.TasksCreateMessageCapability()
                )
                if has_sampling
                else None,
                elicitation=types.TasksElicitationCapability(
                    create=types.TasksCreateElicitationCapability()
                )
                if has_elicitation
                else None,
            )
    ...
    async def handle_request(
        ...
        """
        client_response_type: TypeAdapter[types.ClientResult | types.ErrorData] = (
            TypeAdapter(types.ClientResult | types.ErrorData)
        )
        ...
        match responder.request.root:
            ...
            case _:  # pragma: no cover
                raise ValueError(
                    f"Unhandled request type: {type(responder.request.root)}"
                )
