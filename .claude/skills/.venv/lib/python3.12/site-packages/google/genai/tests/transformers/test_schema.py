...
def test_build_schema_for_list_of_nested_pydantic_schema(client):
    ...
    list_schema = _transformers.t_schema(client, CountryInfoWithCurrency).model_dump()
...
def test_process_schema_order_properties_propagates_into_defs(client, order_properties):
    ...
    _transformers.process_schema(schema, client, order_properties=order_properties)
...
def test_process_schema_with_cycle(client):
    ...
    expected = {
        "type": "OBJECT",
        "properties": {"recursive": {"type": "OBJECT", "properties": {"self": {}}}},
    }
