...
class BaseApiClient:
    ...
    def __init__(
        ...
        if self.vertexai:
            ...
            if (self.api_key or self.location == "global") and not self.custom_base_url:
                self._http_options.base_url = f"https://aiplatform.googleapis.com/"
            elif (
                self.location in _MULTI_REGIONAL_LOCATIONS and not self.custom_base_url
            ):
            ...
            try:
                ...
                if self._use_google_auth_async():
                    ...
                    self._async_client_session_request_args["max_allowed_time"] = (
                        float("inf")
                        if self._http_options.timeout is None
                        else float(self._http_options.timeout)
                    )
    ...
    def _use_google_auth_sync(self) -> bool:
        ...
        return bool(
            ...
            and not (self._http_options.httpx_client or self._http_options.client_args)
        )
    ...
    async def _async_access_token(self) -> Union[str, Any]:
        ...
        if self._credentials:
            return await async_get_token_from_credentials(self, self._credentials)  # type: ignore[no-any-return]
        else:
            raise RuntimeError("Could not resolve API token from the environment")
    ...
    def _request_once(
        ...
        if self._use_google_auth_sync():
            ...
            if self._authorized_session is None:
                ...
                self._authorized_session.configure_mtls_channel(client_cert_source)  # type: ignore[no-untyped-call]
    ...
    async def _async_upload_fd(
        ...
        *,
        ...
        if self._use_aiohttp():  # pylint: disable=g-import-not-at-top
            ...
            return HttpResponse(
                response.headers,
                response_stream=[await response.text()],  # type: ignore[union-attr]
            )
        else:
            # aiohttp is not available. Fall back to httpx.
            while True:
