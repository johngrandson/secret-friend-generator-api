...
def test_constructor_with_http_options():
    ...
    mldev_client = Client(api_key="google_api_key", http_options=mldev_http_options)
    ...
    assert (
        mldev_client.models._api_client.get_read_only_http_options()["api_version"]
        == "v1main"
    )
    ...
    assert (
        mldev_client.models._api_client.get_read_only_http_options()["timeout"] == 10000
    )
    ...
    assert (
        vertexai_client.models._api_client.get_read_only_http_options()["base_url"]
        == "https://{self.location}-aiplatform.googleapis.com/{{api_version}}/"
    )
    assert (
        vertexai_client.models._api_client.get_read_only_http_options()["api_version"]
        == "v1"
    )
    assert (
        vertexai_client.models._api_client.get_read_only_http_options()["headers"][
            "X-Custom-Header"
        ]
...
def test_replay_client_ml_dev_from_env(monkeypatch, use_vertex: bool):
    ...
    assert not client.models._api_client.vertexai
    assert client.models._api_client.api_key == api_key
    assert isinstance(client.models._api_client, replay_api_client.ReplayApiClient)
...
def test_change_client_mode_from_env(monkeypatch, use_vertex: bool):
    ...
    client1 = Client()
    assert isinstance(client1.models._api_client, replay_api_client.ReplayApiClient)
...
def test_client_ssl_context_implicit_initialization():
    client_args, async_client_args = api_client.BaseApiClient._ensure_httpx_ssl_ctx(
        types.HttpOptions()
    )
...
def test_constructor_with_constructor_base_url_overrides_environment_variables(
    ...
    monkeypatch.setenv("GOOGLE_GEMINI_BASE_URL", "https://gemini-env-base-url.com/")
    monkeypatch.setenv("GOOGLE_VERTEX_BASE_URL", "https://vertex-env-base-url.com/")
...
async def test_get_async_auth_lock_basic_functionality():
    ...
    client = Client(vertexai=True, project="fake_project_id", location="fake-location")
...
async def test_get_async_auth_lock_doesnt_block_other_operations():
    ...
    assert total_time < 0.1, (
        f"Operations took too long ({total_time:.3f}s), suggesting blocking occurred"
    )
...
async def test_get_async_auth_lock_double_checked_locking():
    ...
    try:
        ...
        assert lock_creation_count[0] <= 5, (
...
async def test_get_async_auth_lock_memory_efficiency():
    ...
    for _ in range(100):
        ...
        assert (
            client._api_client._async_auth_lock_creation_lock is initial_creation_lock
        )
