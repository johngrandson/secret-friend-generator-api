...
def _get_not_none_fields(model: pydantic.BaseModel) -> list[str]:
    ...
    return [field for field, value in model.model_dump().items() if value is not None]
...
def test_nullable_conversion():
    ...
    assert set(json_schema.type) == set(
        [types.JSONSchemaType("null"), types.JSONSchemaType("string")]
    )
    assert not_none_field_names == ["type"]
...
def test_items_conversion():
    ...
    assert json_schema.items == types.JSONSchema(type=types.JSONSchemaType("string"))
