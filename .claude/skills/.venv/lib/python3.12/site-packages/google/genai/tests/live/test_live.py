...
def test_mldev_from_env(monkeypatch):
    ...
    assert (
        client.aio.live._api_client._http_options.headers["x-goog-api-key"] == api_key
    )
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_session_send_content_dict(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
    client_content = {
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_session_send_content(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_session_send_bytes(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_session_send_blob(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_session_send_realtime_input(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_session_send_tool_response(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_session_send_input_none(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_session_send_error(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_session_receive_text(mock_websocket, vertexai):
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_session_receive_audio(mock_websocket, vertexai):
    mock_websocket.recv = AsyncMock(
        side_effect=[
            (
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_session_receive_tool_call(mock_websocket, vertexai):
    mock_websocket.recv = AsyncMock(
        side_effect=[
            (
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_session_receive_transcription(mock_websocket, vertexai):
    mock_websocket.recv = AsyncMock(
        side_effect=[
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_go_away(mock_websocket, vertexai):
    mock_websocket.recv = AsyncMock(
        side_effect=[
            '{"goAway": {"timeLeft": "10s"}}',
            '{"serverContent": {"turnComplete": true}}',
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_session_resumption_update(mock_websocket, vertexai):
    mock_websocket.recv = AsyncMock(
        side_effect=[
            """{
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_session_start_stream(mock_websocket, vertexai):
...
async def test_bidi_setup_to_api_speech_config(vertexai):
    ...
    if vertexai:
        ...
        expected_result["setup"]["generationConfig"]["speechConfig"] = {
            "voiceConfig": {"prebuilt_voice_config": {"voice_name": "en-default"}},
            "languageCode": "en-US",
        }
    else:
        expected_result["setup"]["model"] = "models/test_model"
    ...
    ) == types.LiveClientMessage._from_response(response=expected_result, kwargs=None)
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_bidi_setup_to_api_with_context_window_compression(vertexai):
    config = types.LiveConnectConfig(
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_bidi_setup_to_api_with_config_tools_function_declaration(vertexai):
    config_dict = {
        "generation_config": {"temperature": 0.7},
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_bidi_setup_to_api_with_config_tools_function_directly(vertexai):
    config_dict = {
        "generation_config": {"temperature": 0.7},
...
async def test_bidi_setup_to_api_with_config_mcp_tools(
    ...
    assert result == expected_result_vertexai if vertexai else expected_result_googleai
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_bidi_setup_to_api_with_config_tools_code_execution(vertexai):
    config_dict = {
...
@pytest.mark.asyncio
async def test_bidi_setup_publishers(vertexai):
    expected_result = {
        "setup": {
            "generationConfig": {
                "responseModalities": [
    ...
    result = await get_connect_message(
        mock_api_client(vertexai=vertexai), model="publishers/google/models/test_model"
    )
...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_bidi_setup_generation_config_warning(vertexai):
    with pytest.warns(
        DeprecationWarning,
...
@pytest.mark.parametrize("vertexai", [True, False])
def test_parse_client_message_blob_dict(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
...
@pytest.mark.parametrize("vertexai", [True, False])
def test_parse_client_message_client_content(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
    result = session._parse_client_message(
...
@pytest.mark.parametrize("vertexai", [True, False])
def test_parse_client_message_client_content_blob(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
    client_content = types.LiveClientContent(
...
@pytest.mark.parametrize("vertexai", [True, False])
def test_parse_client_message_client_content_dict(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
    client_content = types.LiveClientContent(
...
@pytest.mark.parametrize("vertexai", [True, False])
def test_parse_client_message_realtime_input(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
...
@pytest.mark.parametrize("vertexai", [True, False])
def test_parse_client_message_realtime_input_dict(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
    input = types.LiveClientRealtimeInput(
...
@pytest.mark.parametrize("vertexai", [True, False])
def test_parse_client_message_tool_response(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
...
@pytest.mark.parametrize("vertexai", [True, False])
def test_parse_client_message_function_response(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
...
@pytest.mark.parametrize("vertexai", [True, False])
def test_parse_client_message_realtime_tool_response(mock_websocket, vertexai):
    session = live.AsyncSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
    input = types.LiveClientToolResponse(
...
async def test_bidi_setup_to_api_with_api_key(mock_websocket, vertexai):
    ...
    assert "x-goog-api-key" in capture["headers"], (
