...
def test_factory_method_function_response_part_from_bytes():
    my_part = SubFunctionResponsePart.from_bytes(data=b"123", mime_type="image/png")
    assert my_part.inline_data.data == b"123"
...
def test_function_with_tuple_contains_unevaluated_items():
    ...
    assert (
        actual_schema_vertex.parameters_json_schema == expected_parameters_json_schema
    )
...
def test_convert_json_schema_with_cycle():
    json_schema_dict = {
        ...
        "$defs": {
            "Foo": {"type": "object", "properties": {"foo": {"$ref": "#/$defs/Foo"}}}
        },
    }
...
def test_instantiate_response_from_batch_json():
    ...
    assert (
        parsed.candidates[0].citation_metadata.citations[0].uri == "http://someurl.com"
    )
