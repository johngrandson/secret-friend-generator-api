...
@pytest.mark.parametrize("vertexai", [True, False])
@pytest.mark.asyncio
async def test_async_session_control_signal_reset_context(mock_websocket, vertexai):
    session = live_music.AsyncMusicSession(
        api_client=mock_api_client(vertexai=vertexai), websocket=mock_websocket
    )
    if vertexai:
...
async def test_async_session_receive(mock_websocket, vertexai):
    ...
    async for response in session.receive():
        ...
        assert (
            audio_chunk.source_metadata.client_content.weighted_prompts[0].weight == 1
        )
