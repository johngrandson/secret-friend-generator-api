...
test_table: list[pytest_helper.TestTableItem] = [
    ...
    pytest_helper.TestTableItem(
        ...
        parameters=types._GenerateContentParameters(
            ...
            config={"system_instruction": t.t_content("I say high, you say low")},
        ),
    ),
    pytest_helper.TestTableItem(
        name="test_labels",
    ...
    pytest_helper.TestTableItem(
        ...
        parameters=types._GenerateContentParameters(
            model="gemini-2.5-flash-preview-tts",
            contents=t.t_contents('Alice says "Hi", Bob replies with "what\'s up"?'),
            config=types.GenerateContentConfig(
                response_modalities=["audio"],
                speech_config=types.SpeechConfig(
...
def test_multiple_parts(client):
    ...
    response = client.models.generate_content(
        model=GEMINI_FLASH_LATEST,
        contents=[
            types.Part(text="Summarize Shakespeare's life work in a few sentences"),
...
def test_usage_metadata_part_types(client):
    ...
    response = client.models.generate_content(model=GEMINI_FLASH_2_0, contents=contents)
    ...
    modalities = sorted([d.modality.name for d in usage_metadata.prompt_tokens_details])
...
def test_error_handling_stream(client):
    ...
    try:
        ...
        assert (
            e.message == "Developer instruction is not enabled for"
...
def test_provisioned_output_dedicated(client):
    ...
    if client.vertexai:
        assert (
            ...
            == types.TrafficType.PROVISIONED_THROUGHPUT
        )
    else:
