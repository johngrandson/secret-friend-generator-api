...
_BASE64_URL_SAFE = "-_abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
_RAW_BYTES = (
...
def test_base64_pydantic_input_success(
    ...
    assert (
        pytest_helper.get_value_ignore_key_case(
            mock_request_method.call_args[0][2]["contents"][0]["parts"][0], "inlineData"
        )["data"]
        == _BASE64_URL_SAFE
    )
...
def test_base64_pydantic_output_success(client, mock_request_method):
    ...
    assert response.candidates[0].content == types.Content(
        role="model",
        parts=[
            types.Part(inline_data=types.Blob(data=_RAW_BYTES, mime_type="image/png"))
        ],
    )
