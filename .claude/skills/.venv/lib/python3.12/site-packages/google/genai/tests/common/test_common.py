...
def test_recursive_dict_update_type_warnings(
    test_id,
    initial_target,
    update_dict,
    expected_target,
    ...
    if expect_warning:
        ...
        for record in caplog.records:
            if (
                ...
                and expected_log_message_part in record.message
            ):
                pytest.fail(
                    f"Unexpected warning logged for {test_id}: {record.message}"
                )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {
                "config_settings": {"themeName": "dark"}
            },  # Overwrites as per recursive_dict_update logic
        ),
        (
...
def test_repr_max_items():
    ...
    assert "<... 5 more items ...>" in _common._pretty_repr(lst, max_items=5)
    assert "<dict len=10>" in _common._pretty_repr(dct, max_items=5)
    assert "<... 5 more items ...>" in _common._pretty_repr(st, max_items=5)
    assert "<... 5 more items ...>" in _common._pretty_repr(tpl, max_items=5)
...
def test_repr_empty_collections():
    ...
    assert _common._pretty_repr({}) == "{}"
    assert _common._pretty_repr(set()) == "set()"
        ...
        usage_metadata=types.GenerateContentResponseUsageMetadata(
            ...
            prompt_tokens_details=[
                types.ModalityTokenCount(
                    modality=types.MediaModality.TEXT, token_count=7
                )
            ],
...
def test_move_value_by_path():
    ...
    data = {
        "requests": [
            {
                "request": {"content": {"parts": [{"text": "1"}]}},
                "outputDimensionality": 64,
            },
            {
                "request": {"content": {"parts": [{"text": "2"}]}},
                "outputDimensionality": 64,
            },
            {
                "request": {"content": {"parts": [{"text": "3"}]}},
                "outputDimensionality": 64,
            },
        ]
    }
