...
LOCAL_IMAGE_MAN = types.Image.from_file(
    location=os.path.abspath(os.path.join(os.path.dirname(__file__), "../data/man.jpg"))
)

LOCAL_IMAGE_DOG = types.Image.from_file(
    location=os.path.abspath(os.path.join(os.path.dirname(__file__), "../data/dog.jpg"))
)
...
test_table: list[pytest_helper.TestTableItem] = [
    ...
    pytest_helper.TestTableItem(
        ...
        exception_if_vertex=("webhook_config parameter is not supported in Vertex AI"),
    ),
    pytest_helper.TestTableItem(
...
def test_generated_video_extension_from_source_poll(client):
    ...
    operation2 = client.models.generate_videos(
        model=VEO_MODEL_LATEST,
        source=types.GenerateVideosSource(prompt="Sun", video=video1),
        config=types.GenerateVideosConfig(
            number_of_videos=1,
        ),
    )
