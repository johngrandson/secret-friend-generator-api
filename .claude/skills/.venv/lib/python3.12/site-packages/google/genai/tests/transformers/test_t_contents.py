...
def test_content():
    assert t.t_contents([types.Content(parts=[types.Part(text="test")])]) == [
        types.Content(parts=[types.Part(text="test")])
    ]


def test_content_dict():
    assert t.t_contents([{"role": "user", "parts": [{"text": "test"}]}]) == [
...
def test_function_call_part():
    ...
    assert t.t_contents([types.Part(function_call=function_call)]) == [
        types.ModelContent(parts=[types.Part(function_call=function_call)])
    ]
...
def test_empty_string():
    assert t.t_contents("") == [types.UserContent(parts=[types.Part(text="")])]


def test_string():
    assert t.t_contents("test") == [types.UserContent(parts=[types.Part(text="test")])]


def test_file():
    assert t.t_contents(types.File(uri="gs://test", mime_type="image/png")) == [
        types.UserContent(
            parts=[
                types.Part(
...
def test_function_call_list():
    function_call1 = types.FunctionCall(name="test_func1", args={"arg1": "value1"})
    function_call2 = types.FunctionCall(name="test_func2", args={"arg2": "value2"})
...
def test_list_of_text_part_list():
    contents = [
        ...
        "question2",
        types.Part(function_call={"name": "test_func2", "args": {"arg2": "value2"}}),
        {
            "function_response": types.FunctionResponse(
                name="test_func2",
                response={
