...
async def test_function_response_dict(mock_websocket, vertexai):
    ...
    assert (
        sent_data["tool_response"]["functionResponses"][0]["response"]["temperature"]
        == 14.5
    )
    assert sent_data["tool_response"]["functionResponses"][0]["response"]["unit"] == "C"
...
async def test_function_response(mock_websocket, vertexai):
    ...
    assert (
        sent_data["tool_response"]["functionResponses"][0]["response"] == input.response
    )
...
async def test_function_response_scheduling(mock_websocket, vertexai):
    ...
    assert sent_data["tool_response"]["functionResponses"][0]["scheduling"] == "SILENT"
...
async def test_function_response_list(mock_websocket, vertexai):
    ...
    assert (
        sent_data["tool_response"]["functionResponses"][1]["response"]["temperature"]
        == 99.9
    )
