...
def test_join_url_path_base_url_without_path_with_trailing_slash():
    ...
    path = "v1beta/models"
    assert (
        api_client.join_url_path(base_url, path) == "https://fake-url.com/v1beta/models"
    )
...
def test_build_request_with_resource_scope(monkeypatch):
    ...
    assert (
        request.url
        == "https://custom-base-url.com/publishers/google/models/gemini-3-pro-preview"
    )
...
def test_build_request_with_custom_base_url_no_env_vars(monkeypatch):
    request_client = build_test_client_no_env_vars(monkeypatch).models._api_client
    request = request_client._build_request(
        "GET",
        "test/path",
        {"key": "value"},
