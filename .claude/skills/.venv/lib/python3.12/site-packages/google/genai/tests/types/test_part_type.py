...
def test_two_candidates_text(caplog, generate_content_response):
    ...
    assert any(
        record.levelname == "WARNING" and "there are 2 candidates" in record.message
...
def test_thought_signature_no_warning_in_text(caplog, generate_content_response):
    generate_content_response.candidates = [
        types.Candidate(
            content=types.Content(
                parts=[
...
def test_from_file_media_resolution_str():
    ...
    assert (
        file_uri.media_resolution.level
