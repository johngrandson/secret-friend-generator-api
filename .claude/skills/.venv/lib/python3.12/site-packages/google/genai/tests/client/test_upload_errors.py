...
def test_upload_fd_error(client: api_client.BaseApiClient):
    ...
    mock_httpx_client.request.side_effect = [
        _httpx_response(
            200, headers={"X-Goog-Upload-URL": "http://fake/upload"}
        ),  # Initial request to get upload URL
        _httpx_response(
            400, content=error_content, headers={"X-Goog-Upload-Status": "final"}
        ),  # Upload request failing
    ]
    client._httpx_client = mock_httpx_client
...
async def test_async_upload_fd_error_aiohttp(client: api_client.BaseApiClient):
    ...
    mock_aiohttp_session.request = mock.AsyncMock(
        side_effect=[
            _httpx_response(
                200, headers={"X-Goog-Upload-URL": "http://fake/upload"}
            ),  # Initial request
    ...
    with (
        mock.patch.object(client, "_use_aiohttp", return_value=True),
        mock.patch.object(
            client, "_get_aiohttp_session", return_value=mock_aiohttp_session
        ),
        pytest.raises(errors.APIError, match="Unsupported MIME type: bad/mime_type"),
        io.BytesIO(b"test") as f,
    ):
