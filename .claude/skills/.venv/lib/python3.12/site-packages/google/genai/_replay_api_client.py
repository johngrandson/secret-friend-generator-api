...
def _record_on_api_error(
    client: "ReplayApiClient", http_request: HttpRequest
) -> Iterator[None]:
    try:
        yield
...
async def _async_record_on_api_error(
    client: "ReplayApiClient", http_request: HttpRequest
) -> AsyncIterator[None]:
    try:
        yield
...
class ReplayApiClient(BaseApiClient):
    ...
    def _verify_response(self, response_model: BaseModel) -> None:
        ...
        if isinstance(response_model, list):
            response_model = response_model[0]
        _debug_print(f"response_model: {response_model.model_dump(exclude_none=True)}")
        ...
        if not self._private:
            ...
            _debug_print(
                f"Expected SDK response mismatch:\nActual: {actual}\nExpected: {expected}"
            )
