...
AFC_HISTORY = [
    ...
    types.Content(
        ...
        parts=[
            types.Part(
                function_call=types.FunctionCall(name="foo", args={"bar": "baz"})
            )
        ],
    ),
]
...
def mock_generate_content_with_empty_text_part():
    with mock.patch.object(models.Models, "generate_content") as mock_generate_content:
...
def test_history_with_invalid_turns():
    valid_input = types.Content(role="user", parts=[types.Part.from_text(text="Hello")])
    valid_output = [
        types.Content(
            role="model",
            parts=[types.Part.from_text(text="Hello there! how can I help you?")],
    ...
    invalid_input = types.Content(
        role="user",
        parts=[types.Part.from_text(text="a input will be rejected by the model")],
    )
    ...
    models_module = models.Models(mock_api_client)
    chats_module = chats.Chats(modules=models_module)
    chat = chats_module.create(model="gemini-2.5-flash", history=comprehensive_history)
