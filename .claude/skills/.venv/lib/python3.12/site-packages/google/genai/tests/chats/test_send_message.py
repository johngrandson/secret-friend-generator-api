...
def divide_intergers_with_customized_math_rule(numerator: int, denominator: int) -> int:
...
def test_thinking_budget(client):
    ...
    response2 = chat.send_message("can you help me to understand the logic better?")
    has_thought2 = False
...
def test_config_override(client):
    ...
    default_config_response = chat.send_message("tell me a story in 100 words")
...
async def test_async_config_override(client):
    ...
    default_config_response = await chat.send_message("tell me a story in 100 words")
...
async def test_async_stream_config_override(client):
    ...
    async for chunk in await chat.send_message_stream(
        "tell me family friendly story in 100 words"
    ):
        default_config_text += chunk.text
...
def test_mcp_tools_stream(client):
    ...
    for chunk in chat.send_message_stream("What is the weather in Boston?"):
        pass
    for chunk in chat.send_message_stream("What is the weather in San Francisco?"):
        pass
...
async def test_async_mcp_tools_stream(client):
    ...
    async for chunk in await chat.send_message_stream("What is the weather in Boston?"):
        pass
    async for chunk in await chat.send_message_stream(
...
async def test_async_server_side_mcp_tools(client):
    with pytest_helper.exception_if_vertex(client, ValueError):
        ...
        await chat.send_message("What is the weather in San Francisco on 02/02/2026?")
