...
test_table: list[pytest_helper.TestTableItem] = [
    pytest_helper.TestTableItem(
        name="test_register",
        parameters=types._InternalRegisterFilesParameters(
            uris=["gs://unified-genai-dev/image.jpg"]
        ),
        exception_if_vertex="only supported in the Gemini Developer client",
        skip_in_api_mode=(
            "The files have a TTL, they cannot be reliably retrieved for a long time."
        ),
    ),
]
...
async def test_async(client):
    with pytest_helper.exception_if_vertex(client, ValueError):
        files = await client.aio.files._register_files(
            uris=["gs://unified-genai-dev/image.jpg"]
        )
        assert files.files
