...
def mock_api_client(vertexai=False):
    ...
    mock_client._http_options = types.HttpOptions.model_validate({"headers": {}})
...
async def test_receive_server_content(mock_websocket, vertexai):
    ...
    assert (
        result.server_content.model_turn.parts[0].text == "This is a simple response."
    )
    assert result.server_content.turn_complete
    assert result.server_content.grounding_metadata.web_search_queries == ["test query"]
    assert (
        result.server_content.grounding_metadata.grounding_chunks[0].web.domain
        == "google.com"
    )
    assert (
        result.server_content.grounding_metadata.grounding_chunks[0].web.title
        == "Search results"
    )
...
async def test_receive_server_content_with_turn_reason(mock_websocket, vertexai):
    ...
    assert (
        result.server_content.model_turn.parts[0].text == "Please provide more details."
    )
    assert result.server_content.turn_complete is True
    assert (
        result.server_content.turn_complete_reason
        == types.TurnCompleteReason.NEED_MORE_INPUT
    )
