...
class Versions(SyncAPIResource):
    ...
    def create(
        ...
        *,
        ...
        if not skill_id:
            raise ValueError(
                f"Expected a non-empty value for `skill_id` but received {skill_id!r}"
            )
        extra_headers = {
            **strip_not_given(
                {
                    "anthropic-beta": ",".join(
                        chain((str(e) for e in betas), ["skills-2025-10-02"])
                    )
                    if is_given(betas)
                    else not_given
                }
        ...
        extracted_files = extract_files(
            cast(Mapping[str, object], body), paths=[["files", "<array>"]]
        )
        ...
        return self._post(
            path_template(
                "/v1/skills/{skill_id}/versions?beta=true", skill_id=skill_id
            ),
    ...
    def retrieve(
        ...
        *,
        ...
        if not version:
            raise ValueError(
                f"Expected a non-empty value for `version` but received {version!r}"
            )
        extra_headers = {
            **strip_not_given(
                {
        ...
        *,
        ...
        return await self._post(
            ...
            body=await async_maybe_transform(
                body, version_create_params.VersionCreateParams
            ),
            files=extracted_files,
            options=make_request_options(
                extra_headers=extra_headers,
