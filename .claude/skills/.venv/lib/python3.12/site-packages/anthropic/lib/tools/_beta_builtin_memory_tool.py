...
class BetaAbstractMemoryTool(BetaBuiltinFunctionTool):
    ...
    def to_dict(self) -> BetaMemoryTool20250818Param:
        param: BetaMemoryTool20250818Param = {
    ...
    def call(self, input: object) -> BetaFunctionToolResultType:
        command = cast(
            ...
            construct_type_unchecked(
                value=input, type_=cast(Any, BetaMemoryTool20250818Command)
            ),
        )
...
class BetaLocalFilesystemMemoryTool(BetaAbstractMemoryTool):
    ...
    def _validate_path(self, path: str) -> Path:
        ...
        full_path = (
            self.memory_root / relative_path if relative_path else self.memory_root
        )
        ...
        if full_path.is_dir():
            ...
            def collect_items(dir_path: Path, relative_path: str, depth: int) -> None:
                ...
                for item in dir_contents:
                    ...
                    if item.is_dir():
                        items.append(
                            (_format_file_size(stat.st_size), f"{item_relative_path}/")
                        )
                        if depth < 2:
                            collect_items(item, item_relative_path, depth + 1)
                    elif item.is_file():
                        items.append(
                            (_format_file_size(stat.st_size), item_relative_path)
                        )
            ...
            if len(lines) > MAX_LINES:
                raise ToolError(
                    f"File {command.path} exceeds maximum line limit of 999,999 lines."
                )
            ...
            if command.view_range and len(command.view_range) == 2:
                ...
                end_line = (
                    len(lines) if command.view_range[1] == -1 else command.view_range[1]
                )
    ...
    def create(self, command: BetaMemoryTool20250818CreateCommand) -> str:
        ...
        try:
            fd = os.open(
                full_path, os.O_CREAT | os.O_EXCL | os.O_WRONLY, _FILE_CREATE_MODE
            )
            try:
...
async def _async_atomic_write_file(target_path: AsyncPath, content: str) -> None:
    ...
    try:
        ...
        def write_replace_and_sync() -> None:
            fd = os.open(
                sync_temp_path, os.O_CREAT | os.O_EXCL | os.O_WRONLY, _FILE_CREATE_MODE
            )
            try:
                offset = 0
                while offset < len(data):
...
async def _async_validate_no_symlink_escape(
...
class BetaAsyncLocalFilesystemMemoryTool(BetaAsyncAbstractMemoryTool):
    ...
    async def view(self, command: BetaMemoryTool20250818ViewCommand) -> str:
        ...
        if await full_path.is_dir():
            ...
            async def collect_items(
                dir_path: AsyncPath, relative_path: str, depth: int
            ) -> None:
                if depth > 2:
                    return
    ...
    async def rename(self, command: BetaMemoryTool20250818RenameCommand) -> str:
        ...
        await new_full_path.parent.mkdir(
            parents=True, exist_ok=True, mode=_DIR_CREATE_MODE
        )
