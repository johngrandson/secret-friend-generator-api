...
class BetaMessageStreamManager(Generic[ResponseFormatT]):
    ...
    def __enter__(self) -> BetaMessageStream[ResponseFormatT]:
        ...
        self.__stream = BetaMessageStream(
            raw_stream, output_format=self.__output_format
        )
    ...
    *,
    ...
    if current_snapshot is None:
        if event.type == "message_start":
            return cast(
                ParsedBetaMessage[ResponseFormatT],
                ParsedBetaMessage.construct(**cast(Any, event.message.to_dict())),
            )
    ...
    if event.type == "content_block_start":
        ...
        current_snapshot.content.append(
            cast(
                ...
                construct_type(
                    type_=ParsedBetaContentBlock, value=event.content_block.to_dict()
                ),
            ),
        )
    elif event.type == "content_block_delta":
