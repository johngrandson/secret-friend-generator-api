...
class ParsedBetaTextEvent(BaseModel):
    ...
    def parsed_snapshot(self) -> Dict[str, Any]:
        return cast(
            Dict[str, Any],
            jiter.from_json(
                self.snapshot.encode("utf-8"), partial_mode="trailing-strings"
            ),
        )
...
class ParsedBetaMessageStopEvent(
    BetaRawMessageStopEvent, GenericModel, Generic[ResponseFormatT]
):
...
class ParsedBetaContentBlockStopEvent(
    BetaRawContentBlockStopEvent, GenericModel, Generic[ResponseFormatT]
):
