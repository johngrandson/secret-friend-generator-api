...
from ...types.beta.parsed_beta_message import (
    ResponseFormatT,
    ParsedBetaMessage,
...
class BaseToolRunner(Generic[AnyFunctionToolT, ResponseFormatT]):
    ...
    def append_messages(
        self, *messages: BetaMessageParam | ParsedBetaMessage[ResponseFormatT]
    ) -> None:
        ...
        self.set_messages_params(
            lambda params: {
                **params,
                "messages": [*params["messages"], *message_params],
            }
        )
...
class BaseSyncToolRunner(
    BaseToolRunner[BetaRunnableTool, ResponseFormatT],
    ...
    def __init__(
        ...
        *,
        ...
        self._last_message: (
            Callable[[], ParsedBetaMessage[ResponseFormatT]]
    ...
    def _check_and_compact(self) -> bool:
        ...
        threshold = self._compaction_control.get(
            "context_token_threshold", DEFAULT_THRESHOLD
        )
        ...
        # Perform compaction
        log.info(
            f"Token usage {tokens_used} has exceeded the threshold of {threshold}. Performing compaction."
        )
        ...
        messages = [
            *messages,
            BetaMessageParam(
                role="user",
                content=self._compaction_control.get(
        ...
        log.info(
            f"Compaction complete. New token usage: {response.usage.output_tokens}"
        )
        ...
        while not self._should_stop():
            with self._handle_request() as item:
                ...
                if (
                    ...
                    and last_assistant_message.container is not None
                ):
            ...
            if not self._check_and_compact():
                ...
                if response is None:
                    log.debug(
                        "Tool call was not requested, exiting from tool runner loop."
                    )
                    return
    ...
    def _get_last_assistant_message_content(
        self,
    ) -> list[ParsedBetaContentBlock[ResponseFormatT]] | None:
        last_assistant_message = self._get_last_assistant_message()
...
class BetaToolRunner(
    BaseSyncToolRunner[ParsedBetaMessage[ResponseFormatT], ResponseFormatT]
):
    @override
    @contextmanager
...
class BetaStreamingToolRunner(
    BaseSyncToolRunner[BetaMessageStream[ResponseFormatT], ResponseFormatT]
):
    @override
    @contextmanager
...
class BaseAsyncToolRunner(
    BaseToolRunner[BetaAsyncRunnableTool, ResponseFormatT],
    Generic[RunnerItemT, ResponseFormatT],
    ABC,
):
    def __init__(
    ...
    async def _get_last_assistant_message(
        self,
    ) -> ParsedBetaMessage[ResponseFormatT] | None:
        last_message = await self._get_last_message()
        if (
            last_message is None
            or last_message.role != "assistant"
    ...
    async def _get_last_assistant_message_content(
        self,
    ) -> list[ParsedBetaContentBlock[ResponseFormatT]] | None:
        last_assistant_message = await self._get_last_assistant_message()
        if last_assistant_message is None:
...
class BetaAsyncToolRunner(
    BaseAsyncToolRunner[ParsedBetaMessage[ResponseFormatT], ResponseFormatT]
):
    @override
    @asynccontextmanager
    async def _handle_request(
        ...
        message = await self._client.beta.messages.parse(
            **self._params, **self._options
        )
        self._last_message = message
        yield message
...
class BetaAsyncStreamingToolRunner(
    BaseAsyncToolRunner[BetaAsyncMessageStream[ResponseFormatT], ResponseFormatT]
):
    @override
    @asynccontextmanager
    async def _handle_request(
