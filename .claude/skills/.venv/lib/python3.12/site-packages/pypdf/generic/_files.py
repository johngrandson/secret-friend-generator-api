...
class EmbeddedFile:
    ...
    def __init__(
        self, name: str, pdf_object: DictionaryObject, parent: ArrayObject | None = None
    ) -> None:
        """
        Args:
    ...
    @classmethod
    def _create_new(
        cls, writer: PdfWriter, name: str, content: str | bytes
    ) -> EmbeddedFile:
        """
    ...
    def _get_names_array(cls, writer: PdfWriter) -> ArrayObject:
        ...
        if CA.NAMES not in writer.root_object:
            ...
            writer.root_object[NameObject(CA.NAMES)] = writer._add_object(
                DictionaryObject()
            )
    ...
    def alternative_name(self) -> str | None:
        ...
        for key in [
            FileSpecificationDictionaryEntries.UF,
            FileSpecificationDictionaryEntries.F,
        ]:
    ...
    def alternative_name(self, value: TextStringObject | None) -> None:
        ...
        if value is None:
            if FileSpecificationDictionaryEntries.UF in self.pdf_object:
                self.pdf_object[NameObject(FileSpecificationDictionaryEntries.UF)] = (
                    NullObject()
                )
            if FileSpecificationDictionaryEntries.F in self.pdf_object:
                self.pdf_object[NameObject(FileSpecificationDictionaryEntries.F)] = (
                    NullObject()
                )
        else:
    ...
    def description(self, value: TextStringObject | None) -> None:
        """Set the description."""
        if value is None:
            self.pdf_object[NameObject(FileSpecificationDictionaryEntries.DESC)] = (
                NullObject()
            )
        else:
    ...
    def delete(self) -> None:
        ...
        if self.pdf_object in self._parent:
            ...
            indirect_reference := getattr(self.pdf_object, "indirect_reference", None)
        ) is not None and indirect_reference in self._parent:
    ...
    def _load_from_names(cls, names: ArrayObject) -> Generator[EmbeddedFile]:
        ...
        for i, name in enumerate(names):
            if not isinstance(name, str):
                ...
                yield EmbeddedFile(
                    name=direct_name, pdf_object=file_dictionary, parent=names
                )
