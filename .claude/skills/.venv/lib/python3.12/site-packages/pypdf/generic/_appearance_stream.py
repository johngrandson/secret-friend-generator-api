...
class BaseStreamConfig:
    ...
    rectangle: Union[RectangleObject, tuple[float, float, float, float]] = (
        0.0,
        0.0,
        0.0,
        0.0,
...
class TextStreamAppearance(BaseStreamAppearance):
    ...
    def _scale_text(
            ...
            for i, word in enumerate(words):
                ...
                if test_width > field_width and current_line_words:
                    wrapped_lines.append(
                        (current_line_width, " ".join(current_line_words))
                    )
        ...
        estimated_total_height = (
            font_size + (len(wrapped_lines) - 1) * leading_factor * font_size
        )
    ...
    def _generate_appearance_stream_data(
        ...
        leading_factor = (
            font.font_descriptor.bbox[3] - font.font_descriptor.bbox[1]
        ) / 1000.0
        ...
        if font_size == 0:
            ...
            if is_multiline:
                ...
                font_size = round(
                    max(min(max_vertical_size, max_horizontal_size), min_font_size), 1
                )
            ...
            lines = [
                (font.text_width(char) * font_size / 1000, char)
                for index, char in enumerate(text)
                if index < (max_length or len(text))
            ]
        else:
            lines = [
                (font.text_width(line) * font_size / 1000, line)
        ...
        if is_multiline:
            y_offset = (
                rectangle.height
                + margin
                - font.font_descriptor.bbox[3] * font_size / 1000.0
            )
        else:
            y_offset = margin + (
                (field_height - font.font_descriptor.ascent * font_size / 1000) / 2
            )
        ...
        for line_number, (line_width, line) in enumerate(lines):
            ...
            if is_comb and max_length:
                ...
                desired_abs_x_start = (
                    line_number * cell_width
                ) + centering_offset_in_cell
    ...
    def __init__(
        ...
        if font_resource:
            ...
            logger_warning(
                f"Font dictionary for {font_name} not found; defaulting to Helvetica.",
                __name__,
            )
            font_name = "/Helv"
            ...
            font = Font(
                ...
                character_map={},
                encoding=dict(
                    zip(range(256), fill_from_encoding("cp1252"))
                ),  # WinAnsiEncoding
                sub_type="Type1",
    ...
    def _find_annotation_font_resource(
        font_name: str, annotation: DictionaryObject, acro_form: DictionaryObject
    ...
    def from_text_annotation(
        ...
        _rectangle = cast(
            RectangleObject, annotation[AnnotationDictionaryAttributes.Rect]
        )
        rectangle = RectangleObject(
            (
                0,
                0,
                abs(_rectangle[2] - _rectangle[0]),
                abs(_rectangle[3] - _rectangle[1]),
            )
        )
        ...
        if (
            field.get(FieldDictionaryAttributes.FT, "/Tx") == "/Ch"
            and field_flags & FieldDictionaryAttributes.FfBits.Combo == 0
        ):
            text = "\n".join(
                annotation.get_inherited(FieldDictionaryAttributes.Opt, [])
            )
        ...
        font_name, font_resource = cls._find_annotation_font_resource(
            font_name, annotation, acro_form
        )
        ...
        layout = BaseStreamConfig(
            rectangle=rectangle, border_width=border_width, border_style=border_style
        )
        ...
        if AnnotationDictionaryAttributes.AP in annotation:
            for key, value in (
                cast(DictionaryObject, annotation[AnnotationDictionaryAttributes.AP])
                .get("/N", {})
                .items()
            ):
