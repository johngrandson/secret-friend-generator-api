...
class XmpInformation(XmpInformationProtocol, PdfObject):
    ...
    def write_to_stream(
        ...
        deprecate_with_replacement(
            "XmpInformation.write_to_stream", "PdfWriter.xmp_metadata", "6.0.0"
        )
    ...
    def _get_seq_values(
        ...
        for element in self.get_element("", namespace, name):
            if (
                seqs := _generic_get(
                    element, self, list_type="Seq", converter=converter
                )
            ) is not None:
                retval.extend(seqs)
    ...
    def _set_single_value(
        self, namespace: str, name: str, value: Optional[str]
    ) -> None:
    ...
    def _set_bag_values(
        self, namespace: str, name: str, values: Optional[list[str]]
    ) -> None:
    ...
    def _set_langalt_values(
        self, namespace: str, name: str, values: Optional[dict[str, str]]
    ) -> None:
