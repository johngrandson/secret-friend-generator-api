...
class FontDescriptor:
    ...
    bbox: tuple[float, float, float, float] = field(
        default_factory=lambda: (-100.0, -200.0, 1000.0, 900.0)
    )
...
class Font:
    ...
    def _collect_cid_character_widths(
        ...
        for idx, w_entry in enumerate(_w):
            ...
            if not isinstance(w_entry, (int, float)):
                ...
                # sometimes we do.
                logger_warning(
                    f"Expected numeric value for width, got {w_entry}. Ignoring it.",
                    __name__,
                )
                continue
            ...
            # check for format (2): `int int int`
                ...
                logger_warning(
                    f"Invalid font width definition. Last element: {w_entry}.", __name__
                )
    ...
    def _add_default_width(current_widths: dict[str, int], flags: int) -> None:
        ...
        current_widths["default"] = (
            sum(valid_widths) // len(valid_widths) if valid_widths else 500
        )
    ...
    def from_font_resource(
        ...
        if pdf_font_dict.get("/Subtype") in (
            "/Type1",
            "/MMType1",
            "/TrueType",
            "/Type3",
            ...
            if interpretable:  # Save some overhead if font is not interpretable
                ...
                if "/FontDescriptor" in pdf_font_dict:
                    font_descriptor_obj = pdf_font_dict.get(
                        "/FontDescriptor", DictionaryObject()
                    ).get_object()
                    if "/MissingWidth" in font_descriptor_obj:
                        character_widths["default"] = cast(
                            int, font_descriptor_obj["/MissingWidth"].get_object()
                        )
                    font_descriptor = FontDescriptor(
                        **cls._parse_font_descriptor(font_descriptor_obj)
                    )
                    ...
                    bbox_tuple = tuple(
                        map(float, cast(ArrayObject, pdf_font_dict["/FontBBox"]))
                    )
                    assert len(bbox_tuple) == 4, bbox_tuple
            ...
            for d_font_idx, d_font in enumerate(
                ...
                cast(ArrayObject, pdf_font_dict["/DescendantFonts"])[d_font_idx] = (
                    d_font
                )
                ...
                font_descriptor_obj = d_font.get(
                    "/FontDescriptor", DictionaryObject()
                ).get_object()
                font_descriptor = FontDescriptor(
                    **cls._parse_font_descriptor(font_descriptor_obj)
    ...
    def text_width(self, text: str = "") -> float:
        ...
        return sum(
            [
                self.character_widths.get(char, self.character_widths["default"])
                for char in text
            ],
            0.0,
        )
