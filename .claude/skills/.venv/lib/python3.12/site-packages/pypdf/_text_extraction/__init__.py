...
def crlf_space_check(
    ...
    try:
        if abs(moved_height) > 0.8 * min(
            str_height * scale_prev_y, font_size * scale_y
        ):
            if (output + text)[-1] != "\n":
                output += text + "\n"
                ...
                text = ""
        elif (moved_width >= (spacewidth + str_widths) * scale_prev_x) and (
            output + text
        )[-1] != " ":
            text += " "
    except Exception:
    ...
    if orientation in orientations and len(operands) > 0:
        if isinstance(operands[0], str):
            ...
            if isinstance(font.encoding, str):
                ...
                t = "".join(
                    [
                        font.encoding[x] if x in font.encoding else bytes((x,)).decode()
                        for x in tt
                    ]
                )
