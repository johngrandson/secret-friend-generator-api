...
class TestMainCLI:
    ...
    def test_generate_image_dispatch(self, mock_key, mock_gen):
        ...
        with patch("sys.argv", ["cli", "--task", "generate", "--prompt", "A cat"]):
            cli.main()
    ...
    def test_generate_speech_dispatch(self, mock_key, mock_gen):
        ...
        with patch(
            "sys.argv", ["cli", "--task", "generate-speech", "--text", "Hello world"]
        ):
            cli.main()
        mock_gen.assert_called_once()
    ...
    def test_speech_uses_text_or_prompt(self, mock_key, mock_gen):
        ...
        with patch(
            "sys.argv",
            ["cli", "--task", "generate-speech", "--prompt", "Fallback text"],
        ):
            cli.main()
    ...
    def test_generate_music_dispatch(self, mock_key, mock_gen):
        ...
        with patch(
            "sys.argv", ["cli", "--task", "generate-music", "--lyrics", "La la la"]
        ):
            cli.main()
        mock_gen.assert_called_once()
    ...
    def test_generate_video_dispatch(self, mock_key, mock_gen):
        ...
        with patch(
            "sys.argv", ["cli", "--task", "generate-video", "--prompt", "A dancer"]
        ):
            cli.main()
        mock_gen.assert_called_once()
    ...
    def test_auto_model_detection(self, mock_key):
        with patch("sys.argv", ["cli", "--task", "generate-speech", "--text", "hi"]):
    ...
    def test_exception_exits_with_1(self, mock_key, mock_gen):
        ...
        with patch("sys.argv", ["cli", "--task", "generate", "--prompt", "test"]):
            with pytest.raises(SystemExit) as exc_info:
                cli.main()
            assert exc_info.value.code == 1
