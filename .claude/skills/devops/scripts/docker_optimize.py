...
class DockerfileAnalyzer:
        ...
        for i, line in enumerate(self.lines, 1):
            ...
            if line.startswith("FROM"):
                ...
                if ":latest" in line or (
                    ": " not in line and "AS" not in line and "@" not in line
                ):
                    self.issues.append(
                        {
                            "line": i,
    ...
    def analyze_layer_caching(self) -> None:
        ...
        has_package_copy = any(
            "package.json" in line or "requirements.txt" in line or "go.mod" in line
            for _, line in copy_lines
        )
        has_source_copy = any(
            "COPY . ." in line or "COPY ./" in line for _, line in copy_lines
        )
    ...
    def analyze_security(self) -> None:
        ...
        has_user = any(
            line.strip().startswith("USER") and "root" not in line.lower()
            for line in self.lines
        )
    ...
    def analyze(self) -> Dict:
        ...
        return {
            ...
            "summary": {
                ...
                "warnings": len(
                    [i for i in self.issues if i.get("severity") == "warning"]
                ),
...
def main():
    ...
    parser.add_argument("dockerfile", type=str, help="Path to Dockerfile")
    ...
    try:
        analyzer = DockerfileAnalyzer(
            dockerfile_path=args.dockerfile, verbose=args.verbose
        )
