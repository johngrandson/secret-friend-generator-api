...
from src.use_cases.user.create import CreateUserRequest, CreateUserUseCase
...
def use_case(repo, publisher):
    return CreateUserUseCase(user_repository=repo, event_publisher=publisher)


async def test_create_user_success(use_case, repo, publisher):
    repo.find_by_email.return_value = None
    saved_user = User.create(email=Email("new@example.com"), name="Alice")
    repo.save.return_value = saved_user

    resp = await use_case.execute(CreateUserRequest(email="new@example.com", name="Alice"))
...
async def test_create_user_publisher_receives_user_created_event(use_case, repo, publisher):
    repo.find_by_email.return_value = None
...
async def test_create_user_duplicate_email(use_case, repo, publisher):
    repo.find_by_email.return_value = User.create(
        email=Email("dup@example.com"), name="Existing"
    )

    resp = await use_case.execute(CreateUserRequest(email="dup@example.com", name="New"))
...
async def test_create_user_invalid_email(use_case, repo, publisher):
...
async def test_create_user_blank_name(use_case, repo, publisher):
    repo.find_by_email.return_value = None
